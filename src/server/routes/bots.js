  ...
  if (!err && !result) {
   try {
    ...
    await fetch(`https://discord.com/api/v7/users/${req.body.id}`, {
     ...
     if (!err && req.body.github) {
      //if (!req.body.github.match(gitregex)) err = "invalid_github"
      if((!req.body.github.startsWith("https://github.com/")) && (!req.body.github.startsWith("https://www.github.com/")) && (!req.body.github.startsWith("https://gitlab.com/"))) err="invalid_github"; //lemme add my dank memer bot
     }
     if (!err && req.body.desc) {
      req.body.desc = coronaSanitizer(req.body.desc, {
       allowedTags: coronaSanitizer.defaults.allowedTags.concat(['discord-message', 'discord-messages', 'img', 'iframe', 'style', 'h1', 'h2', 'link', 'mark']),
       allowVulnerableTags: true,
       allowedAttributes: {
        '*': ["*"]
       }
