...
schedule.scheduleJob("15 * * * *", async function () {
  Cache.Bots.find({}).then((bots) => {
    for (var bot of bots) {
      const moneh = Math.floor(Math.random() * (bot.servers / (24 * 7))) +
      Math.floor(Math.random() * 10);
      bot.votes += moneh;
      if(bot.webhook){
        //find users who have bots
        let us = [];
        Cache.Bots.find({}).then(async (bots) => {
          for (var bot of bots) {
            //push all the bot owners to us
        ...
        //remove duplicates from owner
        us = [...new Set(us)];
        //get all the users
        let uss = [];
        Cache.Users.find({}).then(async (users) => {
          for (var user of users) {
            uss.push(user.id);
          }
        });
        //remove us array from uss
        uss = uss.filter((x) => !us.includes(x));
        //get random user
        const random = uss[Math.floor(Math.random() * uss.length)];
        ue = Cache.Users.findOne({id: uss[random]});
        const hmm = JSON.stringify({
          user: Cache.Users.clean(uss._doc),
