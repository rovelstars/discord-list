 ...
 if(publicbot.cooldownearn.has(res.locals?.user?.id)){
  ...
  Users.findOne({id: res.locals?.user?.id}).then(user=>{
    if(!user) res.json({err:"not_logined"});
     ...
     act = privatebot.guilds.cache.get("602906543356379156").members.cache.get(res.locals?.user?.id).presence.activities.filter(e=>{return (e.type=="CUSTOM" && (e.state.includes("dscrdly.com") || e.state.includes("discord.rovelstars.com")))});
