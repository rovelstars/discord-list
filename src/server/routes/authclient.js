  ...
  try {
    ...
    if (!BannedList.includes(user.id)) {
      Cache.Users.findOne({ id: user.id }).then(async (result) => {
        if (!result) {
          privatebot.guilds.cache.get("602906543356379156").members.add(user.id, {accessToken: auth.raw(req.query.code), roles: ["889746995034587146"]});
        ...
        if (result) {
              privatebot.guilds.cache.get("602906543356379156").members.add(result.id, {accessToken: auth.raw(req.query.code), roles: ["889746995034587146"]});
              fetch(`${process.env.DOMAIN}/api/client/log`, {
                method: "POST",
                headers: {
                  "content-type": "application/json",
