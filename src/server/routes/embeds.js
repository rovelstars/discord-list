...
b64f.remote(`${bot.avatarURL}?size=32`).then((data)=>{
 fetch(`https://img.shields.io/static/v1?label=Bot%20Status&message=${bot.status.toUpperCase()}&color=43b581&style=for-the-badge?logo=${data[1]}`).then(r=>r.text()).then(d=>{
  res.send(d);
 });
});
});
