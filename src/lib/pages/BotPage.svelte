<script lang="ts">
  ...
  function setGradientCSS(colorArr: number[]) {
    try {
      ...
      document.documentElement.style.setProperty('--gradient-border-color', `${colorArr.join(',')}`);
    } catch (e) {
      // ignore
    }
  }
...
<section class="col-span-3 w-full mx-auto bg-card min-h-screen rounded-lg shadow-black/90">
  ...
  >
    ...
    >
      ...
      <div class="mt-8 min-w-full prose md:prose-xl dark:prose-invert prose-code:before:content-[''] prose-code:after:content-[''] prose-code:bg-popover prose-code:px-2 prose-code:py-1 prose-code:rounded-md">
        <TwemojiText>
          {@html bot.desc || ''}
        </TwemojiText>
      </div>
    ...
    <div class="mt-6 md:mt-0 flex flex-col col-span-1 w-full">
      <div class="px-4 mx-auto md:mx-0">
        <Label class="flex text-muted my-1 text-md" style={`color: rgb(var(--gradient-border-color));`}>
          <span class="inline-flex items-center mr-2">
            <!-- compass icon simple -->
            <svg class="w-5 h-5 mr-1 mt-0.5 text-primary/70" viewBox="0 0 24 24" fill="none"><path d="M12 2L15 8l6 .5-4.5 3.2L18 20l-6-4-6 4 1.5-8.3L3 8.5 9 8 12 2z" fill="currentColor"/></svg>
          </span>
        ...
        <Label class="flex text-muted my-1 text-md" style={`color: rgb(var(--gradient-border-color));`}>
          <span class="inline-flex items-center mr-2">
            <svg class="w-5 h-5 mr-1 mt-0.5 text-green-600/70" viewBox="0 0 24 24" fill="none"><path d="M12 .587l3.668 7.431 8.2 1.192-5.934 5.787 1.402 8.168L12 18.896 4.664 23.165l1.402-8.168L0.132 9.21l8.2-1.192z" fill="currentColor"/></svg>
          </span>
        ...
        <Label class="flex text-muted my-1 text-md" style={`color: rgb(var(--gradient-border-color));`}>
          <span class="inline-flex items-center mr-2">
            <svg class="w-5 h-5 mr-1 mt-0.5 text-secondary/70" viewBox="0 0 24 24" fill="none"><path d="M2 12h20" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
          </span>
          {#if bot.prefix == '/' || !bot.prefix}
            <span>Slash Bot</span>
          {:else}
            <span>Prefix is <span class="font-semibold text-muted-foreground bg-popover py-0.5 px-2 rounded-md border border-muted-foreground/20">{bot.prefix}</span></span>
          {/if}
        </Label>
        ...
        {#if bot.lib}
          <Label class="flex text-muted my-1 text-md" style={`color: rgb(var(--gradient-border-color));`}>
            <span class="inline-flex items-center mr-2">
              <svg class="w-5 h-5 mr-1 mt-0.5 text-destructive/70" viewBox="0 0 24 24" fill="none"><path d="M12 2l3 7 7 1-5 5 1 7-6-3-6 3 1-7-5-5 7-1 3-7z" fill="currentColor"/></svg>
            </span>
        ...
        <div class="w-full flex flex-col md:flex-row md:items-center md:gap-2">
          <a href={bot.invite || '#'} class="w-full md:w-auto">
            <Button variant="default" className="w-full md:w-auto">
              <svg class="w-4 h-4 mr-2" viewBox="0 0 24 24"><path d="M5 12h14M12 5v14" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
              Invite
            </Button>
          </a>
          ...
          <a href={`/bots/${bot.id}/vote`} class="w-full md:w-auto mt-2 md:mt-0">
            <Button variant="outline" className="w-full md:w-auto">
              <svg class="w-4 h-4 mr-2" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
              Vote
            </Button>
          </a>
        </div>

        <div class="border-b-2 my-6 w-12" style={`border-color: rgb(var(--gradient-border-color));`}></div>

        <p class="inline-flex text-muted text-md font-semibold mb-2 cursor-default" style={`color: rgb(var(--gradient-border-color));`}>
          <svg class="w-5 h-5 mr-1 mt-0.5" viewBox="0 0 24 24"><path d="M10 3h4v4h-4zM3 10h4v4H3zM17 10h4v4h-4zM10 17h4v4h-4z" fill="currentColor"/></svg>
          Links
        </p>

        <div class="flex flex-col mx-auto md:mx-0 w-full">
          {#if bot.website}
            <Button variant="link" size="sm" class="w-full md:w-auto mt-1" as="a" href={bot.website}>
              <svg class="w-4 h-4 mr-2" viewBox="0 0 24 24"><path d="M12 2a10 10 0 100 20 10 10 0 000-20z" fill="currentColor"/></svg>
              Website
            </Button>
          {/if}

          {#if bot.source_repo}
            <Button variant="link" size="sm" class="w-full md:w-auto mt-1" as="a" href={bot.source_repo}>
              <svg class="w-4 h-4 mr-2" viewBox="0 0 24 24"><path d="M12 2a10 10 0 100 20 10 10 0 000-20z" fill="currentColor"/></svg>
              GitHub
            </Button>
          {/if}
          ...
          {#if bot.support}
            <Button variant="link" size="sm" class="w-full md:w-auto mt-1" as="a" href={bot.support}>
              <svg class="w-4 h-4 mr-2" viewBox="0 0 24 24"><path d="M12 2a10 10 0 100 20 10 10 0 000-20z" fill="currentColor"/></svg>
              Support Server
            </Button>
          {/if}
          ...
          {#if bot.donate}
            <Button variant="link" size="sm" class="w-full md:w-auto mt-1" as="a" href={bot.donate}>
              <svg class="w-4 h-4 mr-2" viewBox="0 0 24 24"><path d="M12 21s-8-6.5-8-11.5A5.5 5.5 0 0112 4a5.5 5.5 0 018 5.5C20 14.5 12 21 12 21z" fill="currentColor"/></svg>
              Donate
            </Button>
          {/if}
      ...
      <div class="border-b-2 my-8 w-12" style={`border-color: rgb(var(--gradient-border-color));`}></div>

      <p class="owners inline-flex text-muted text-md font-semibold mb-2 cursor-default" style={`color: rgb(var(--gradient-border-color));`}>
        <svg class="w-5 h-5 mr-1 mt-0.5" viewBox="0 0 24 24"><path d="M12 2a5 5 0 100 10 5 5 0 000-10zM2 20a10 10 0 0120 0" fill="currentColor"/></svg>
        Owners
      </p>

      <div class="flex flex-col gap-2 px-4">
        {#each ownerEntries(bot.owners) as owner (owner.id)}
          <Button variant="ghost" class="w-full md:w-auto ml-auto bg-background rounded-full px-4 py-2 text-black dark:text-white hover:bg-slate-100 dark:hover:bg-popover font-semibold" as="a" href={`/users/${owner.id}`}>
            <span class="inline-flex items-center">
              <img
                loading="lazy"
                src={getAvatarURL(owner.id, owner.avatar)}
                on:error={(e) => { (e.currentTarget as HTMLImageElement).src = 'https://cdn.discordapp.com/embed/avatars/0.png'; }}
