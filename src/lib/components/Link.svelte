...
{#if !isExternal}
	...
	<a {href} class={className} {id} aria-label={ariaLabel} {...$$restProps}>
		<slot />
	</a>
{:else}
	...
	{#if $isDesktop}
		...
		<Dialog bind:open ariaLabel="External link confirmation">
			...
			<div slot="content" class="max-w-sm mx-auto">
				...
				<div class="flex gap-2 justify-end">
					<button
						class="inline-flex items-center gap-2 rounded-md text-sm font-medium bg-primary text-primary-foreground px-4 py-2"
						on:click={onOpenClick}
					>
						Open
					</button>
					<button
						class="inline-flex items-center gap-2 rounded-md text-sm font-medium border border-input bg-background px-4 py-2"
						on:click={close}
					>
						Cancel
					</button>
	...
	{:else}
		...
		{#if open}
			<div class="fixed inset-0 z-50 flex items-end">
				...
				<!-- sheet -->
				<div
					class="relative w-full bg-popover border-t rounded-t-lg p-4 z-50 animate-slide-up"
					role="dialog"
					aria-modal="true"
					aria-label="External link popup"
				>
					<div class="mb-2">
					...
					<div class="flex gap-2">
						<button
							class="flex-1 inline-flex items-center justify-center gap-2 rounded-md text-sm font-medium bg-primary text-primary-foreground px-4 py-2"
							on:click={onOpenClick}
						>
							Open
						</button>
						<button
							class="flex-1 inline-flex items-center justify-center gap-2 rounded-md text-sm font-medium border border-input bg-background px-4 py-2"
							on:click={close}
						>
							Cancel
						</button>
