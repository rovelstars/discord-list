 ...
 */
function resolveKey(request: Request, cookies: { get(name: string): string | undefined }): string | null {
...
export const POST: RequestHandler = async ({ request, params, cookies }) => {
	...
	const text: string | null = typeof body.text === 'string' ? body.text.trim().slice(0, 2000) || null : null;
	...
	if (parentId === null) {
		...
		if (ratingRaw == null) {
			return json({ err: 'rating_required', message: 'A rating is required for top-level reviews.' }, { status: 400 });
		}
		// Validate rating range and precision
		if (!isFinite(ratingRaw) || ratingRaw < 0.5 || ratingRaw > 5) {
			return json({ err: 'invalid_rating', message: 'Rating must be between 0.5 and 5.0.' }, { status: 400 });
		}
		if (Math.round(ratingRaw * 10) !== ratingRaw * 10) {
			return json({ err: 'invalid_rating', message: 'Rating may have at most one decimal place (e.g. 4.3).' }, { status: 400 });
		}
		...
		const topLevelExisting = (existing ?? []).filter((r: any) => r.parent_id == null || r.parent_id === undefined);
		...
		if (hasTopLevel) {
			return json({ err: 'already_reviewed', message: 'You have already reviewed this bot. Edit your existing review instead.' }, { status: 409 });
		}
	} else {
		...
		if (parent.parent_id !== null) {
			return json({ err: 'cannot_nest_replies', message: 'Replies can only be made to top-level comments.' }, { status: 400 });
		}
		// Must provide at least some text in a reply
		if (!text) {
			return json({ err: 'text_required', message: 'Reply text cannot be empty.' }, { status: 400 });
		}
	}
