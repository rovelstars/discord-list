 ...
 *
 * All /dashboard/** routes REQUIRE server-side authentication. Unlike the rest
 * of the site (which defers auth to the client for cacheability), dashboard
 * pages contain sensitive per-user data that must be validated before any HTML
 * is sent.
 *
 * This layout load:
 *   1. Checks for the `key` cookie
 *   2. Validates the token against Discord's API (with refresh fallback)
 *   3. Redirects to /login if the user is not authenticated
 *   4. Returns the authenticated user object for all child routes
 */
