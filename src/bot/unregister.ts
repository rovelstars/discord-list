...
export default async function unregisterCommands(env: {
	...
	const registeredCommands: any[] = (await rest.get(
		Routes.applicationCommands(env.DISCORD_BOT_ID)
	)) as any[];
	const registeredGuildCommands: any[] = (await rest.get(
	...
	for (const command of registeredGuildCommands) {
		...
		await rest.delete(
			Routes.applicationGuildCommand(env.DISCORD_BOT_ID, env.DISCORD_GUILD_ID, command.id)
		);
	}
}
