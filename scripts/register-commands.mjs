...
async function main() {
	...
	log(`Found ${c.bold}${commands.length}${c.reset} command(s): ${commands.map(cmd => c.cyan + cmd.name + c.reset).join(', ')}`);
	log(`Mode: ${c.bold}${mode}${c.reset} → registering ${isGuild ? `as ${c.pink}guild${c.reset} commands on guild ${c.bold}${guildId}${c.reset}` : `${c.pink}globally${c.reset}`}`);
	...
	const names = Array.isArray(registered)
		? registered.map(cmd => `${c.cyan}${cmd.name}${c.reset} (id: ${c.dim}${cmd.id}${c.reset})`).join('\n    ')
		: '(no list returned)';

	ok(`Successfully registered ${c.bold}${Array.isArray(registered) ? registered.length : '?'}${c.reset} command(s) ${isGuild ? `${c.pink}to guild ${guildId}` : `${c.pink}globally`}${c.reset}:`);
	ok(`    ${names}`);
}
